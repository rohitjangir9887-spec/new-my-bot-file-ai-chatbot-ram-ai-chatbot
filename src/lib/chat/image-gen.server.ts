...
export async function generateImageServer(prompt: string, aspectRatio: '1:1' | '16:9' | '9:16' = '1:1', conversationId?: string) {
  ...
  try {
    const aiResponse = await fetch('https://api.lovable.ai/v1/images/generations', {
      ...
      body: JSON.stringify({ prompt: prompt.trim(), model: process.env.LOVABLE_IMAGE_MODEL?.trim() || 'dall-e-3', n: 1, size: aspectRatio === '1:1' ? '1024x1024' : aspectRatio === '16:9' ? '1792x1024' : '1024x1792', response_format: 'b64_json' }),
    ...
    const { error: uploadError } = await supabaseAdmin.storage.from('user-files').upload(storagePath, buffer, { contentType: 'image/png', cacheControl: '3600', upsert: false });
    if (uploadError) throw new Error('IMAGE_STORAGE_FAILED');
    const { data: fileRecord, error: dbError } = await supabaseAdmin.from('user_files').insert({ user_id: user.id, filename: `Generated image - ${prompt.trim().slice(0, 80)}`, storage_path: storagePath, mime_type: 'image/png', size_bytes: buffer.length }).select().single();
    if (dbError) { await supabaseAdmin.storage.from('user-files').remove([storagePath]); throw new Error('IMAGE_STORAGE_FAILED'); }
