...
export function getFallbackModels(selectedId: string) {
  ...
  if (!selected) return MODEL_CATALOG.filter(m => m.enabled);
  return [selected, ...MODEL_CATALOG.filter(m => m.enabled && m.id !== selected.id && m.capabilities.includes('chat'))];
}

function endpointFor(model: ModelDefinition) {
  if (model.provider === 'nvidia') return 'https://integrate.api.nvidia.com/v1/chat/completions';
  if (model.provider === 'lovable') return 'https://api.lovable.ai/v1/chat/completions';
  return 'https://api.openai.com/v1/chat/completions';
}

function keyFor(model: ModelDefinition) {
  if (model.provider === 'nvidia') return env('NVIDIA_API_KEY');
  if (model.provider === 'lovable') return env('LOVABLE_AI_API_KEY');
...
async function callOpenAICompatible(model: ModelDefinition, messages: any[], tools: any[], signal?: AbortSignal) {
  ...
  if (!key) throw Object.assign(new Error('Provider unavailable'), { code: 'PROVIDER_UNAVAILABLE', status: 503 });
  const response = await fetch(endpointFor(model), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${key}` },
    body: JSON.stringify({ model: model.providerModelId, messages, tools, tool_choice: tools.length ? 'auto' : 'none', stream: false }),
    signal,
  });
...
async function callAnthropic(model: ModelDefinition, messages: any[], tools: any[], signal?: AbortSignal) {
  ...
  const input = messages.filter(m => m.role !== 'system').map(m => ({ role: m.role === 'assistant' ? 'assistant' : 'user', content: m.content }));
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-api-key': key, 'anthropic-version': '2023-06-01' },
    body: JSON.stringify({ model: model.providerModelId, max_tokens: 4096, system: system || undefined, messages: input, tools: tools.length ? tools.map(t => ({ name: t.function.name, description: t.function.description, input_schema: t.function.parameters })) : undefined }),
    signal,
  });
...
export async function callModel(model: ModelDefinition, messages: any[], tools: any[], signal?: AbortSignal) {
  if (model.provider === 'anthropic') return callAnthropic(model, messages, tools, signal);
  return callOpenAICompatible(model, messages, tools, signal);
}
