...
const requestSchema = z.object({
  conversationId: z.string().uuid(), requestId: z.string().uuid(), modelId: z.string().min(1),
  webSearchEnabled: z.boolean().optional().default(true), memoryEnabled: z.boolean().optional().default(true), safeSearch: z.boolean().optional().default(true),
  messages: z.array(z.object({ role: z.enum(['user', 'assistant']), content: z.string().max(100_000), attachments: z.array(z.unknown()).optional() })).min(1).max(50),
});
...
function jsonResponse(body: unknown, status = 200) { return new Response(JSON.stringify(body), { status, headers: { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' } }); }

function sseResponse(content: string, meta: Record<string, unknown>) {
  const encoder = new TextEncoder();
  const stream = new ReadableStream({ start(controller) { try { for (let i = 0; i < content.length; i += 80) controller.enqueue(encoder.encode(`data: ${JSON.stringify({ choices: [{ delta: { content: content.slice(i, i + 80) } }] })}\n\n`)); controller.enqueue(encoder.encode(`data: ${JSON.stringify({ meta })}\n\n`)); controller.enqueue(encoder.encode('data: [DONE]\n\n')); } finally { controller.close(); } } });
...
async function runWebSearch(query: string, safeSearch: boolean) {
  ...
  try {
    const response = await fetch('https://api.tavily.com/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ api_key: key, query: query.trim(), search_depth: 'basic', max_results: 5, include_answer: false, safe_search: safeSearch }), signal: AbortSignal.timeout(8000) });
    if (!response.ok) return { error: 'Live web search is currently unavailable.' };
    const body: any = await response.json().catch(() => null);
    const results = Array.isArray(body?.results) ? body.results.filter((result: any) => typeof result?.title === 'string' && typeof result?.url === 'string').slice(0, 5).map((result: any) => ({ title: result.title, url: result.url, content: typeof result.content === 'string' ? result.content.slice(0, 4000) : '' })) : [];
...
async function executeTool(name: string, args: any, conversationId: string, safeSearch: boolean) {
  ...
  if (name === 'generate_image') {
    try {
      ...
      return { image: { id: result.fileId, name: 'Generated image', type: 'image', mimeType: result.mimeType, size: result.sizeBytes, url: result.url, storagePath: result.storagePath, status: 'ready' }, prompt: result.prompt };
    } catch { return { error: 'Image generation is currently unavailable.' }; }
  }
  return { error: 'Tool unavailable.' };
}

export const Route = createFileRoute('/api/chat/stream')({ server: { handlers: { POST: async ({ request }) => {
  try {
    ...
    const { data: persistedUser } = await supabaseAdmin.from('messages').select('id, content').eq('conversation_id', body.conversationId).eq('role', 'user').order('created_at', { ascending: false }).limit(1).maybeSingle();
    if (!persistedUser || persistedUser.content !== lastMessage.content) return jsonResponse({ error: publicError() }, 409);

    const existing = await supabaseAdmin.from('messages').select('id, content, metadata, attachments').eq('conversation_id', body.conversationId).eq('role', 'assistant').filter('metadata->>requestId', 'eq', body.requestId).maybeSingle();
    if (existing.data?.content?.trim()) {
      const metadata = (existing.data.metadata as Record<string, unknown>) || {};
      return sseResponse(existing.data.content, { messageId: existing.data.id, model: metadata.model, provider: metadata.provider, usage: metadata.usage, attachments: existing.data.attachments || metadata.attachments || [], status: 'completed' });
    }
    ...
    const { data: profile } = await supabaseAdmin.from('profiles').select('custom_instructions, response_tone, response_length, personalization_enabled, memory_enabled, web_search_enabled, safe_search').eq('id', user.id).maybeSingle();
    const personalizationEnabled = profile?.personalization_enabled !== false;
    const memoryEnabled = body.memoryEnabled && profile?.memory_enabled !== false;
    const webSearchEnabled = body.webSearchEnabled && profile?.web_search_enabled !== false;
    const safeSearch = body.safeSearch && profile?.safe_search !== false;
    const memories = memoryEnabled ? await retrieveMemories(user.id, lastMessage.content) : [];
    const contextBlocks: string[] = [];
    if (personalizationEnabled && profile?.custom_instructions?.trim()) contextBlocks.push(`User custom instructions:\n${profile.custom_instructions.trim()}`);
    if (personalizationEnabled && profile?.response_tone) contextBlocks.push(`Preferred response tone: ${profile.response_tone}`);
    if (personalizationEnabled && profile?.response_length) contextBlocks.push(`Preferred response length: ${profile.response_length}`);
    ...
    const tools = webSearchEnabled ? allTools : allTools.filter(tool => tool.function.name !== 'web_search');
    ...
    let result: any = null; let usedModel = selected; let usedToolCalls: any[] = []; let attachments: any[] = []; let lastStatus = 503;
      ...
      try {
        ...
        for (let round = 0; round < 4; round++) {
          ...
          for (const call of result.toolCalls.slice(0, 8)) {
            ...
            const toolResult = toolUsage.allowed ? await executeTool(call.function?.name || '', args, body.conversationId, safeSearch) : { error: 'Tool limit reached for this plan.' };
    ...
    const metadata = { requestId: body.requestId, model: usedModel.id, provider: usedModel.provider, status: 'completed', usage: result.usage, toolCalls: usedToolCalls, attachments };
