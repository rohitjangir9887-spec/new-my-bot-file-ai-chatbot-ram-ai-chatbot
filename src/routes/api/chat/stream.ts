...
const requestSchema = z.object({
  conversationId: z.string().uuid(), modelId: z.string().min(1), webSearchEnabled: z.boolean().optional().default(true), memoryEnabled: z.boolean().optional().default(true),
...
async function executeTool(name: string, args: any, conversationId: string) {
  ...
  if (name === 'generate_image') { try { const result = await generateImageServer(String(args?.prompt || ''), args?.aspectRatio || '1:1', conversationId); return { image: { id: result.fileId, name: 'Generated image', type: 'image', mimeType: result.mimeType, size: result.sizeBytes, url: result.url, storagePath: result.storagePath, status: 'ready' }, prompt: result.prompt }; } catch { return { error: 'Image generation is currently unavailable.' }; }
  }
...
export const Route = createFileRoute('/api/chat/stream')({ server: { handlers: { POST: async ({ request }) => {
  try {
    const authHeader = request.headers.get('Authorization'); if (!authHeader?.startsWith('Bearer ')) return new Response(JSON.stringify({ error: publicError() }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    const { data: { user }, error: authError } = await supabaseAdmin.auth.getUser(authHeader.slice(7)); if (authError || !user) return new Response(JSON.stringify({ error: publicError() }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    ...
    const selected = getModel(body.modelId); if (!selected) return new Response(JSON.stringify({ error: publicError() }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    const plan = await getUserPlan(user.id); if (planRank(plan) < planRank(selected.plan)) return new Response(JSON.stringify({ error: 'This model requires a paid plan.' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
    const usage = await consumeUsage(user.id, 'message', body.modelId); if (!usage.allowed) return new Response(JSON.stringify({ error: 'Your plan limit has been reached. Upgrade your plan to continue.' }), { status: 429, headers: { 'Content-Type': 'application/json' } });
    ...
    for (const candidate of candidates) {
      try {
        let messages: any[] = [...systemContext, ...body.messages.map(m => ({ role: m.role, content: m.content }))]; result = null; usedToolCalls = []; attachments = [];
        for (let round = 0; round < 3; round++) {
          result = await callModel(candidate, messages, tools, request.signal); if (!result.toolCalls?.length) break;
          ...
          for (const call of result.toolCalls) {
            ...
            usedToolCalls.push({ id: call.id, type: call.function?.name, status: toolResult?.error ? 'error' : 'completed', args, result: toolResult }); if (toolResult?.image) attachments.push(toolResult.image);
            messages.push({ role: 'tool', tool_call_id: call.id, content: JSON.stringify(toolResult) });
          }
        }
        if (result?.content?.trim()) { usedModel = candidate; break; } lastStatus = 502;
    ...
    const content = result.content.trim(); const metadata = { model: usedModel.id, provider: usedModel.provider, status: 'completed', usage: result.usage, toolCalls: usedToolCalls, attachments };
    ...
    const status = error?.name === 'ZodError' ? 400 : 500; console.error('Chat request failed', { status, code: error?.code });
